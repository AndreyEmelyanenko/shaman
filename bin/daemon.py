...
class Daemon:
    """
    Daemon is main class of shaman which is used for start|stop|restart workers. It also controlls arguments for
    input, daemonizing and etc. To get help, please use command:
    ...
    def init_config(self):
        ...
        from shamanapp.helpers.daemon_exceptions import SigtermDaemonException
        import shamanapp.abstract_message as abstract_message
    ...
    def parse_config(self, config):
        from shamanapp.helpers.configuration_change import CustomParser
    ...
    def modify_config_according_to_cliargs(self, initial_config, cliArgs):
        from shamanapp.helpers.configuration_change import ignore_after, ignore_fields, is_config_correct
...
if __name__=='__main__':
    ...
    cli = argparse.ArgumentParser(description = 'Main shaman module. Use it to start|stop|restart daemon or start non-daemon modes of shaman', formatter_class = argparse.ArgumentDefaultsHelpFormatter)

    cli.add_argument('command', choices=['stop', 'start', 'restart', ''], help='Command to daemon', nargs="*",
                     default='')

    group = cli.add_mutually_exclusive_group()
    group.add_argument('-i', dest='stdin_input', help='Use stdin input as main input', action='store_true', default=False)
