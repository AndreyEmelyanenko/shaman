...
# from helpers.crawler_exceptions import CrawlerKafkaSendMaximumRetriesExceeded
...
from abstract_message import Message, control_messages, MessageFormat

from helpers.graphite import GraphiteSenderTimer
import sys
...
class Consumer(object):
    def __init__(self, basepath, config, worker_num, in_queue):
        ...
        for dir in self._consumer_config['GENERAL']['stages_dir'].split(';'):
            sys.path.append(basepath + "/" + dir)
        sys.path.append(basepath + "/shamanapp/analyzers/general_stages")
        sys.path.append(basepath + "/shamanapp/analyzers/input_stages")
        sys.path.append(basepath + "/shamanapp/analyzers/output_stages")

        self._read_graphite_config()
    ...
    def _sort_stages(self):
        ...
        for name in self.imported_classes.keys():
            ...
            if self.graphite_enabled:
                analyzer_do_stage_graphite = GraphiteSenderTimer(self.graphite_host,
                                                                 self.graphite_port,
                                                                 self.graphite_global_key,
                                                                 self.name,
                                                                 metrics_name=name,
                ...
                analyzer_obj.do_stage = analyzer_do_stage_graphite.timeit_graphite(analyzer_obj.do_stage)
